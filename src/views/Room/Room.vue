...
const roomOwner = computed(() => clientRoomState.value?.owner || null);
const usersList = computed(() => clientRoomState.value?.users || []);
const isUserOwner = computed(() => clientState.value?.user?.id ? roomOwner.value === clientState.value.user.id : false);
...
const syncRoom = async () => {
    ...
    if (!initialized.value) {
        ...
        const videoUrl = isTorrentStream
            ? await StremioService.createTorrentStream(stream)
            : stream.url;
...
const syncPlayer = () => {
    if (playerState.value.autoSync) {
        ClientService.send('player.sync', {
            ...
            time: playerState.value.video?.currentTime || 0,
        });
    }
};
...
onMounted(() => {
    ...
    syncPlayerInterval = setInterval(() => {
        if (playerState.value.video && !playerState.value.paused) syncPlayer();
    }, 1000);
});
...
.room {
    ...
    &.chat-open .users-list { display: none; }
...
@media only screen and (min-width: 768px) and (min-height: 768px) {
    .room.chat-open .users-list { display: inherit; }
}
</style>
